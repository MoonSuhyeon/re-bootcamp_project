...
type Child = { label: string; href: string; exact?: boolean; disabled?: boolean }
...
const NAV: SidebarItem[] = [
  ...
  },
  {
    ...
    expandable: true,
    children: [
      { label: '진행현황조회/실행/예약', href: '/products/loan/status',            exact: true },
      { label: '사후서류제출',           href: '/products/loan/status/docs',       exact: true },
      { label: '배우자정보제공동의',     href: '/products/loan/status/spouse',     exact: true, disabled: true },
      { label: '세대원정보제공동의',     href: '/products/loan/status/household',  exact: true, disabled: true },
      { label: '제3자담보정보제공동의',   href: '/products/loan/status/collateral', exact: true, disabled: true },
      { label: '부동산담보대출 전자서명', href: '/products/loan/status/sign',       exact: true },
    ],
  },
  {
    label: '대출관리',
  ...
  return (
    <aside className="w-[180px] flex-shrink-0 border-r border-kb-border min-h-[700px] pt-6 pr-2">
      ...
      <div className="mt-4 space-y-2 pr-1">
        <Link href="/cert" className="flex items-center gap-2 border border-kb-border px-3 py-2.5 text-[13px] text-kb-text-body hover:bg-kb-beige-light transition-colors">
          <span className="text-base">🔒</span>
          <span className="font-medium">인증센터</span>
        </Link>
        <div className="border border-kb-border px-3 py-2.5">
          <div className="flex items-center gap-2 mb-0.5">
            <span className="text-base">📞</span>
            <span className="text-[13px] font-bold text-kb-text">신규상담</span>
          </div>
          <p className="text-[13px] font-bold text-kb-text pl-7">1800-9500</p>
        </div>
      </div>
    </aside>
  )
