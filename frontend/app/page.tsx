    ...
    <header className="bg-white">
      <div className="max-w-kb-container mx-auto px-6 flex items-center justify-between h-[70px]">
        ...
        <nav className="flex items-center gap-0 text-body">
          ...
          <Link href="/products" className="px-3 text-kb-text-muted hover:text-kb-text transition-colors">금융상품</Link>
          <span className="text-kb-border mx-1">|</span>
...
// ── 빠른조회 모달 ──
const QUICK_TABS = ['빠른조회', '자기앞/어음 수표 사고신고조회', '제증명발급조회', 'ID조회', '상조예치금 조회', '가상계좌 조회']

function QuickInquiryModal({ onClose }: { onClose: () => void }) {
  const [tab, setTab] = useState(0)
  const [acctNo, setAcctNo] = useState('')
  const [password, setPassword] = useState('')
  const [mouseInput, setMouseInput] = useState(false)
  const [searchType, setSearchType] = useState<'id' | 'biz'>('id')
  const [searchVal, setSearchVal] = useState('')
  const [period, setPeriod] = useState('당일')
  const [content, setContent] = useState('전체')
  const [order, setOrder] = useState('최근')
  ...
  const PERIODS = ['당일', '3일', '1주일', '1개월', '3개월', '6개월']

  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center">
      <div className="absolute inset-0 bg-black/40" onClick={onClose} />
      <div className="relative bg-white w-[760px] shadow-xl">
        {/* 헤더 */}
        <div className="bg-kb-yellow px-5 py-3 flex items-center justify-between">
          <span className="text-[16px] font-bold text-kb-text">빠른조회</span>
          <span className="text-[13px] font-bold text-kb-text flex items-center gap-1">
            <svg viewBox="0 0 20 20" fill="none" className="w-4 h-4"><path d="M10 2L3 7v6c0 4 2.5 7 7 8 4.5-1 7-4 7-8V7L10 2z" fill="#1A1A1A"/></svg>
            AX풀뱅크
          </span>
        </div>
        ...
        <div className="flex border-b border-kb-border">
          {QUICK_TABS.map((t, i) => (
            <button key={i} onClick={() => setTab(i)}
              className={`px-4 py-2.5 text-[12px] whitespace-nowrap border-b-2 -mb-px transition-colors ${
                tab === i ? 'border-kb-text font-bold text-kb-text' : 'border-transparent text-kb-text-muted hover:text-kb-text'
              }`}>
              {t}
            </button>
          ))}
        ...
        <div className="p-5">
          {tab !== 0 ? (
            <p className="text-center py-10 text-[13px] text-kb-text-muted">서비스 준비 중입니다.</p>
          ) : (
            <>
              {/* 안내 */}
              <div className="text-[12px] text-kb-text-body mb-4 space-y-0.5">
                <p>· 사전에 영업점이나 인터넷뱅킹으로 등록하신 계좌에 대해서만 조회 가능합니다.
                  <button className="ml-1 text-kb-blue underline">빠른조회 등록하기</button>
                </p>
                <p>· 개인 또는 개인사업자는 생년월일, 법인사업자는 사업자등록번호로 조회 가능합니다.</p>
              </div>

              {/* 폼 */}
              <table className="w-full text-[13px] border-t border-kb-text border-collapse">
                <tbody>
                  {/* 계좌번호 */}
                  <tr>
                    <td className="bg-[#F5F5F5] border border-kb-border px-4 py-2.5 font-medium text-kb-text w-[120px] whitespace-nowrap">계좌번호</td>
                    <td className="border border-kb-border px-4 py-2.5">
                      <div className="flex items-center gap-2">
                        <input type="text" value={acctNo} onChange={e => setAcctNo(e.target.value)}
                          className="border border-kb-border px-2 py-1 text-[13px] w-44 outline-none focus:border-kb-text" />
                        <span className="text-[12px] text-kb-text-muted flex items-center gap-1">
                          <span className="inline-flex w-4 h-4 rounded-full border border-kb-border items-center justify-center text-[9px]">i</span>
                          - 없이 숫자만 입력하세요.
                        </span>
                      </div>
                    </td>
                  ...
                  {/* 계좌비밀번호 */}
                  <tr>
                    <td className="bg-[#F5F5F5] border border-kb-border px-4 py-2.5 font-medium text-kb-text whitespace-nowrap">계좌비밀번호</td>
                    <td className="border border-kb-border px-4 py-2.5">
                      <div className="flex items-center gap-3">
                        <input type={mouseInput ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
                          maxLength={4} disabled={mouseInput}
                          className="border border-kb-border px-2 py-1 text-[13px] w-28 outline-none disabled:bg-[#F5F5F5]" />
                  ...
                  {/* 검색구분 */}
                  <tr>
                    <td className="bg-[#F5F5F5] border border-kb-border px-4 py-2.5 font-medium text-kb-text whitespace-nowrap">검색구분</td>
                    <td className="border border-kb-border px-4 py-2.5 space-y-2">
                      <div className="flex items-center gap-2">
                        <label className="flex items-center gap-1.5 cursor-pointer">
                          <input type="radio" checked={searchType === 'id'} onChange={() => setSearchType('id')} className="accent-kb-text" />
                          <span>인터넷뱅킹ID(빠른조회ID)</span>
                        </label>
                        {searchType === 'id' && (
                          <>
                            <input type="text" value={searchVal} onChange={e => setSearchVal(e.target.value)}
                              className="border border-kb-border px-2 py-1 text-[13px] w-32 outline-none" />
                            <button className="border border-kb-border px-3 py-1 text-[12px] text-kb-text hover:bg-kb-beige-light">ID조회</button>
                          </>
                        )}
                      </div>
                      <div className="flex items-center gap-2">
                        <label className="flex items-center gap-1.5 cursor-pointer">
                          <input type="radio" checked={searchType === 'biz'} onChange={() => setSearchType('biz')} className="accent-kb-text" />
                          <span>사업자번호(생년월일)</span>
                        </label>
                        {searchType === 'biz' && (
                          <input type="text" value={searchVal} onChange={e => setSearchVal(e.target.value)}
                            className="border border-kb-border px-2 py-1 text-[13px] w-32 outline-none" />
                  ...
                  {/* 조회조건 */}
                  <tr>
                    <td className="bg-[#F5F5F5] border border-kb-border px-4 py-2.5 font-medium text-kb-text whitespace-nowrap">조회조건</td>
                    <td className="border border-kb-border px-4 py-2.5 space-y-2">
                      <div className="flex gap-1">
                        {PERIODS.map(p => (
                          <button key={p} onClick={() => setPeriod(p)}
                            className={`px-3 py-1 text-[12px] border transition-colors ${period === p ? 'bg-kb-yellow border-kb-border font-bold' : 'border-kb-border text-kb-text-body hover:bg-kb-beige-light'}`}>
                            {p}
                          </button>
                        ))}
                      </div>
                      <div className="flex items-center gap-1 text-[12px]">
                        <select className="border border-kb-border px-1 py-1 text-[12px] outline-none"><option>{yyyy}</option></select>년
                        <select className="border border-kb-border px-1 py-1 text-[12px] outline-none"><option>{mm}</option></select>월
                        <select className="border border-kb-border px-1 py-1 text-[12px] outline-none"><option>{dd}</option></select>일
                        <button className="border border-kb-border px-1.5 py-1 text-[11px] text-kb-text-muted hover:bg-kb-beige-light">📅</button>
                  ...
                  {/* 조회내용 */}
                  <tr>
                    <td className="bg-[#F5F5F5] border border-kb-border px-4 py-2.5 font-medium text-kb-text whitespace-nowrap">조회내용</td>
                    <td className="border border-kb-border px-4 py-2.5">
                      <div className="flex gap-5">
                        {['전체', '입금내용만', '출금내용만'].map(v => (
                          <label key={v} className="flex items-center gap-1.5 cursor-pointer text-[13px]">
                            <input type="radio" checked={content === v} onChange={() => setContent(v)} className="accent-kb-text" />
                            {v}
                          </label>
                        ))}
                      </div>
                  ...
                  {/* 조회결과 순서 */}
                  <tr>
                    <td className="bg-[#F5F5F5] border border-kb-border px-4 py-2.5 font-medium text-kb-text whitespace-nowrap">조회결과 순서</td>
                    <td className="border border-kb-border px-4 py-2.5">
                      <div className="flex gap-5">
                        {['최근 거래내역이 위로', '과거 거래내역이 위로'].map(v => (
                          <label key={v} className="flex items-center gap-1.5 cursor-pointer text-[13px]">
                            <input type="radio" checked={order === v.slice(0, 2)} onChange={() => setOrder(v.slice(0, 2))} className="accent-kb-text" />
                            {v}
                          </label>
                        ))}
                      </div>
              ...
              <div className="flex justify-center my-4">
                <button className="px-12 py-2 text-[14px] font-bold text-kb-text bg-kb-yellow hover:brightness-95">조회</button>
              </div>

              {/* 하단 안내 */}
              <div className="bg-[#F5F5F5] border border-kb-border px-4 py-3 text-[12px] text-kb-text-body space-y-0.5">
                <p>· 입출금이 자유로운 원화계좌 및 외화계좌의 최근 6개월간의 거래내역 및 잔액 조회가 가능합니다.</p>
                <p>· 특전금전신탁 MMT계좌의 최근 6개월간의 거래내역 및 잔액 조회가 가능합니다.</p>
                <p>· 6개월 이상 조회를 원하시는 고객님께서는 가까운 지점을 방문하여 조회하시거나 인터넷뱅킹에 가입하여 로그인 후 조회하셔야 합니다.</p>
              </div>
            </>
          )}
        </div>
        ...
        <div className="flex justify-end px-5 pb-4">
          <button onClick={onClose} className="text-[13px] text-kb-text-muted hover:text-kb-text">닫기 ×</button>
        </div>
      </div>
    </div>
  )
...
function QuickMenuBar() {
  ...
  useEffect(() => {
    try { setIsLoggedIn(!!localStorage.getItem('user')) } catch {}
  }, [])

  function handleAuth(dest: 'accounts' | 'transfer' | 'quick') {
    if (!isLoggedIn) { router.push('/login'); return }
    if (dest === 'accounts') router.push('/inquiry/accounts')
    else if (dest === 'transfer') router.push('/transfer/account')
    else setQuickOpen(true)
  }

  const OTHER_MENUS = ['보안센터', '고객우대제도', '소비자보호', '상담/예약', '상품공시실']
    ...
    <>
      <section>
        <div className="max-w-kb-container mx-auto px-6">
          <div className="flex">
            {(['전체계좌조회', '계좌이체', '빠른조회'] as const).map((label, i) => (
              <button key={label}
                onClick={() => handleAuth((['accounts', 'transfer', 'quick'] as const)[i])}
                className="flex-1 py-5 text-center text-base font-semibold bg-kb-yellow text-kb-text hover:bg-kb-yellow-dark transition-colors duration-kb">
                {label}
              </button>
            ))}
            {OTHER_MENUS.map(label => (
              <Link key={label} href="#"
                className="flex-1 py-5 text-center text-base font-semibold text-kb-text hover:bg-kb-beige-light border-l border-kb-border transition-colors duration-kb">
                {label}
              </Link>
            ))}
          </div>
      ...
      {quickOpen && <QuickInquiryModal onClose={() => setQuickOpen(false)} />}
    </>
  )
}
