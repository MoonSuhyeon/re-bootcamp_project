...
// ⚠️ 데모용 공용 비밀번호.
// 이 관리자 콘솔은 백엔드 RBAC(역할 기반 접근제어)를 "프론트에서 시연"하는 화면이다.
// 실제 인가·감사는 서버(customer-service)에서 강제된다:
//   - JWT의 행위자(employeeId)를 게이트웨이가 X-Employee-Id로 전파
//   - 민감 조회마다 customer_access_log 감사 기록
// 따라서 아래 비교는 인증이 아니라 "데모 진입 게이트"일 뿐이며, 비번은 화면에 힌트로 노출한다.
const DEMO_LOGIN_PASSWORD = 'admin1234'
...
export default function AdminLoginPage() {
  ...
  function handleLogin() {
    ...
    // 데모 진입 게이트 (실제 인증 아님 — 상단 주석 참고)
    if (password !== DEMO_LOGIN_PASSWORD) {
      setError(`비밀번호가 올바르지 않습니다. (데모 비밀번호: ${DEMO_LOGIN_PASSWORD})`)
      return
    }
