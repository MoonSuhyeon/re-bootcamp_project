...
const MOCK_PRODUCTS = [
  ...
  { id: 4, badge: '스타뱅킹', badgeColor: 'gold',  name: '한국주택금융공사 아낌e보금자리론', desc: '한국주택금융공사 보증, 장기 고정금리 주택담보대출', limit: '3.6억원', rate: '연 3.2% ~ 4.1%' },
            ...
            <div className="border-t border-kb-text divide-y divide-kb-border">
              {MOCK_PRODUCTS.map(product => (
                <div key={product.id} className="py-5 flex items-center gap-5">
                  <Link href={`/products/loan/mortgage/${product.id}`} className="flex-1 min-w-0 group">
                    <div className="flex items-center gap-2 mb-1.5">
                      <span className={`text-[11px] font-bold px-2 py-0.5 ${BADGE_STYLE[product.badgeColor]}`}>{product.badge}</span>
                      <span className="text-[16px] font-bold text-kb-text group-hover:underline">{product.name}</span>
                    </div>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <Link href={`/products/loan/mortgage/${product.id}`} className="px-5 py-2 text-[14px] font-bold text-kb-text bg-kb-yellow hover:bg-kb-yellow-dark transition-colors whitespace-nowrap">바로하기</Link>
                  </div>
                </div>
              ))}
            </div>
