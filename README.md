...
### 회원가입·본인확인 → 로그인 (인증·신원확인)

휴대폰 본인인증(SMS)으로 명의자를 확인하고 **온라인 회원가입 → 로그인**까지, 신원확인부터 인증까지의 한 흐름.
- 전체 흐름은 아래 GIF + **장면별 설명**: [`docs/demo/auth/`](docs/demo/auth/)

![회원가입·본인확인·로그인 데모](docs/demo/auth/auth_demo.gif)

> 약관 동의 → 휴대폰 본인인증 → ID 중복확인 → 가입 완료 → 로그인. **"신원확인"이 인증보안계 서사의 출발점**이다.
...
|---|---|---|---|
| ![full](docs/demo/rbac/rbac_1_audit_full.png) | ![masked](docs/demo/rbac/rbac_2_risk_masked.png) | ![reason](docs/demo/rbac/rbac_3_reason_modal.png) | ![audit](docs/demo/rbac/rbac_4_audit_log.png) |
...
- 전체 흐름은 아래 GIF + **장면별 설명**: [`docs/demo/rbac/`](docs/demo/rbac/)

![RBAC 차등접근 & 감사로그 데모](docs/demo/rbac/rbac_demo.gif)
...
- CLI 추론 트레이스(텍스트)·설계: [`agent/README.md`](agent/README.md)
